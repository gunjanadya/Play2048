...
public class Board {
    ...
    public void updateOpenSpaces() {
        //WHY IS THIS HERE??? WHAT AM I SUPPOSED TO USE IT FOR???
        BoardSpot openSpaces[][] = new BoardSpot[gameBoard.length][gameBoard[0].length];
        
        for(int i = 0; i < gameBoard.length; i++){
            for(int j = 0; j <gameBoard[i].length; j++){
                if(gameBoard[i][j] == 0){
                    this.openSpaces.add(new BoardSpot(i,j));
                }
            }
        }
    }
    ...
    public void addRandomTile() {
        //these keep returning zeros instead of actual random numbers
        int spot     = StdRandom.uniform(openSpaces.size());
        double value = StdRandom.uniform(0.0, 1.0);
        int v;
        BoardSpot t  = openSpaces.get(spot);
        if(value < 0.1){
            v = 4;
        }
        else{
    ...
    public void swipeLeft() {
        for(int r = 0; r < 2; r++){
            for(int i = 0; i < gameBoard.length; i++){
                for(int j = 0; j < gameBoard[i].length; j++){
                    if(gameBoard[i][j] == 0){
                        for(int k = j; k < gameBoard[i].length-1; k++){
                            gameBoard[i][k] = gameBoard[i][k+1];
                        }
                        gameBoard[i][gameBoard[i].length-1] = 0;
                    }
                }
            }
        }
    }
    // public boolean checkLeadingZeros(int row){
    //     boolean result = false;
    //     for(int i = 0; i < gameBoard[row].length; i++){
    //         if(gameBoard[row][i] == 0 && !result){
    //             result = true;
    //         }
    //     }
    //     return result;
    ...
    public void transpose() {
        int trans[][] = new int[gameBoard[0].length][gameBoard.length];
        for(int i = 0; i <gameBoard.length; i++){
            for(int j = 0; j <gameBoard[i].length; j++){
                trans[j][i] = gameBoard[i][j];
            }
        }
        gameBoard = trans;
    }
    ...
    public void flipRows() {
        /** so this just. does not work. oh well lol */
        int trans[][] = new int[gameBoard.length][gameBoard[0].length];
        for(int i = 0; i < gameBoard.length; i++){
            for(int j = 0; j < gameBoard[i].length; j++){
                trans[i][gameBoard[i].length-j-1] = gameBoard[i][j];
            }
        }
        gameBoard = trans;
    }
